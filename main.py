...
def orders_to_excel(order_tab, filename):
    ...
    mylist = []
    for cell in ws['V']:  # COLUMN V contains the quantities
        mylist.append(cell.value)
    mylist = mylist[1:]  # remove "КОЛИЧЕСТВО"

    for count, val in enumerate(mylist):
        ws["V" + str(count + 2)] = int(
            val)  # +2 bc COUNT starts from 0, but Excel starts from 1 (need to skip 1st row)
